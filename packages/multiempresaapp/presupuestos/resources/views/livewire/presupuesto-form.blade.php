        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div class="w-full max-w-lg rounded-lg bg-white shadow-xl">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-medium text-gray-900">Buscar concepto</h3>
                    <button type="button" wire:click="closeServicioSearchModal" class="text-gray-400 hover:text-gray-600">
                            ...
                            @foreach ($servicioSearchModalResults as $srv)
                                <li>
                                    <button type="button"
                                        ...
                                        <p class="text-sm font-medium text-gray-900">{{ $srv['nombre'] }}</p>
                                        <p class="text-xs text-gray-500 mt-0.5">{{ number_format($srv['precio'], 2, ',', '.') }} € · IVA {{ $srv['iva_tipo'] !== null ? $srv['iva_tipo'] . '%' : '—' }}</p>
                                    </button>
                                </li>
                            @endforeach
                        </ul>
                    @elseif(strlen($servicioSearchModalQuery) >= 1)
                        <div class="px-6 py-4 text-sm text-gray-500">No se encontraron conceptos con "{{ $servicioSearchModalQuery }}".</div>
                    @else
                        <div class="px-6 py-4 text-sm text-gray-400">Escribe para buscar conceptos...</div>
                    @endif
                </div>
