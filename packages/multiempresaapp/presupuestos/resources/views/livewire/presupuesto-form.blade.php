    ...
    <span class="hidden sm:inline">{{ $label['desktop'] ?? $label }}</span>
        ...
        @if($step === 1)
            <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                ...
                @if($plantillaIdAplicada && $plantillaNombreAplicada)
                <div class="border-t border-slate-100 px-6 py-3">
                    <div class="flex items-center gap-2 rounded-2xl border border-violet-200 bg-violet-50 px-4 py-2.5">
                        <svg class="h-4 w-4 shrink-0 text-violet-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                        </svg>
                        <p class="text-sm text-violet-800">Plantilla aplicada: <span class="font-semibold">{{ $plantillaNombreAplicada }}</span></p>
                    </div>
                </div>
                @endif
                ...
                <div class="flex justify-end px-6 pb-5">
                    ...
                    <button type="button" wire:click="openSelectPlantillaModal"
                        ...
                        {{ $plantillaIdAplicada ? 'Cambiar plantilla' : 'Usar plantilla' }}
                    </button>
                    @endif
