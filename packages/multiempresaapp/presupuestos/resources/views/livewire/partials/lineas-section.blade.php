...
<div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
    ...
    <div class="hidden md:block">
        <div class="overflow-x-auto px-4 pb-4 pt-2 sm:px-6">
            <table class="min-w-full border-separate border-spacing-y-3" style="overflow: visible">
                <thead>
                    <tr>
                ...
                <tbody>
                    @foreach ($lineas as $i => $linea)
                        <tr wire:key="linea-{{ $i }}" class="rounded-2xl bg-slate-50 shadow-sm ring-1 ring-slate-200 transition hover:bg-white hover:shadow-md hover:ring-indigo-100">
                            {{-- Concepto con búsqueda de conceptos --}}
                            <td class="rounded-l-2xl px-4 py-4" x-data="{ open{{ $i }}: @entangle('lineaDropdownVisible.'.$i) }">
                                <div class="relative">
                                    @if(!empty($linea['concepto']) && !empty($linea['servicio_id']))
                                        <div class="flex items-center gap-1 rounded-2xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-700">
                                            ...
                                            <button type="button"
                                                    ...
                                                    class="text-xs leading-none text-slate-400 transition hover:text-slate-600">&times;</button>
                                        </div>
                                    @else
                                        <div class="flex gap-2">
                                            ...
                                            <button type="button"
                                                    ...
                                                    class="inline-flex items-center rounded-2xl border border-slate-300 bg-white px-2.5 py-2 text-slate-500 transition hover:bg-slate-50 hover:text-indigo-600 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                                        ...
                                        @if(!empty($lineaDropdownVisible[$i]))
                                            <div class="absolute z-50 mt-2 w-full min-w-48 rounded-2xl border border-slate-200 bg-white shadow-xl">
    ...
    <div class="space-y-3 p-4 md:hidden">
        @foreach ($lineas as $i => $linea)
            <div class="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm" wire:key="linea-card-{{ $i }}">
                ...
                {{-- Concepto --}}
                <div class="mt-3" x-data="{ open{{ $i }}: @entangle('lineaDropdownVisible.'.$i) }">
                ...
                <div class="mt-3 flex justify-between border-t border-slate-100 pt-3 text-sm">
                    <span class="text-slate-500">Base imp.: <span class="text-slate-700">{{ number_format($linea['base_imponible'] ?? 0, 2, ',', '.') }} €</span></span>
                    <span class="font-semibold text-slate-900">Total: {{ number_format($linea['total'] ?? 0, 2, ',', '.') }} €</span>
                </div>
            </div>
        @endforeach
    </div>
