    ...
    <x-slot name="header">
        <div class="flex items-start gap-3 sm:items-center">
            ...
            <div class="min-w-0">
                ...
                <p class="mt-1 truncate text-sm text-slate-500">{{$cliente->nombre }}</p>
            </div>
        </div>
    </x-slot>
                ...
                <form method="POST" action="{{ route('admin.clientes.update', $cliente) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="notas"
                                  name="notas"
                                  rows="4"
                                  class="block w-full rounded-2xl border-slate-300 px-4 py-3 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('notas') border-rose-300 @enderror">{{ old('notas', $cliente->notas) }}</textarea>
                        @error('notas')
                            <p class="mt-1 text-sm text-rose-600">{{ $message }}</p>
                        @enderror
                    </div>
