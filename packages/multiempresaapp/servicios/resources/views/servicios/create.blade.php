<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-start gap-3 sm:items-center">
            ...
            <div class="min-w-0">
                <h2 class="text-2xl font-bold text-slate-900">Nuevo concepto</h2>
                <p class="mt-0.5 text-sm text-slate-500">Añade un nuevo servicio, producto u otro concepto</p>
            </div>
        </div>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('admin.servicios.store') }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea id="descripcion"
                                      name="descripcion"
                                      rows="3"
                                      class="mt-1 block w-full rounded-2xl border-slate-300 px-4 py-2.5 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('descripcion') border-rose-300 @enderror">{{ old('descripcion') }}</textarea>
                            @error('descripcion')
                                <p class="mt-1 text-sm text-rose-600">{{ $message }}</p>
                            @enderror
                        </div>
