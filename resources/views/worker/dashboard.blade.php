                        ...
                        <table class="min-w-full divide-y divide-gray-100">
                            ...
                            <tbody class="divide-y divide-gray-50 bg-white">
                                @foreach($recentNotas as $nota)
                                    <tr class="hover:bg-gray-50/60">
                                        <td class="whitespace-nowrap px-5 py-3 text-sm font-medium text-gray-900">{{ $nota->titulo }}</td>
                                        <td class="whitespace-nowrap px-5 py-3 text-sm text-gray-600">{{ $nota->cliente?->nombre ?? '—' }}</td>
                                        <td class="whitespace-nowrap px-5 py-3">
                                        ...
                                        <td class="whitespace-nowrap px-5 py-3 text-sm text-gray-500">{{ $nota->created_at->format('d/m/Y') }}</td>
                                        <td class="whitespace-nowrap px-5 py-3 text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                <a href="{{ route('admin.notas.show', $nota->id) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-indigo-700 bg-indigo-50 rounded-lg hover:bg-indigo-100 transition">Ver</a>
                                                <a href="{{ route('admin.notas.edit', $nota->id) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-amber-700 bg-amber-50 rounded-lg hover:bg-amber-100 transition">Editar</a>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
                        ...
                        @foreach($recentNotas as $nota)
                            <div class="p-4 space-y-2">
                                <div>
                                    ...
                                    <p class="mt-0.5 text-xs text-gray-500">{{ $nota->cliente?->nombre ?? '—' }} · {{ $nota->created_at->format('d/m/Y') }}</p>
                                    @if($nota->presupuesto_id)
                                        <a href="{{ route('admin.presupuestos.show', $nota->presupuesto_id) }}" class="mt-1 inline-flex items-center gap-1 text-xs text-indigo-600 font-medium">
