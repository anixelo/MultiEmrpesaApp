<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Editar Plan: {{ $plan->name }}</h1>
                <p class="text-sm text-gray-500 mt-0.5">Modifica los detalles del plan</p>
            </div>
        </div>
    </x-slot>
            ...
            <form method="POST" action="{{ route('superadmin.plans.update', $plan) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="description" rows="2"
                              class="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $plan->description) }}</textarea>
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Máx. empresas activas <span class="text-gray-400 font-normal">(0 = ilimitado)</span></label>
                    <input type="number" name="max_empresas" value="{{ old('max_empresas', $plan->max_empresas ?? 1) }}" min="0"
                           class="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm focus:ring-indigo-500 focus:border-indigo-500 @error('max_empresas') border-red-400 @enderror">
                    @error('max_empresas')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
                ...
                <div class="flex items-center gap-3">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="has_notes" value="1" {{ old('has_notes', $plan->has_notes) ? 'checked' : '' }} class="sr-only peer">
                    ...
                    <span class="text-sm font-medium text-gray-700">Permite gestión de notas</span>
                </div>

                <div class="flex items-center gap-3">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="active" value="1" {{ old('active', $plan->active) ? 'checked' : '' }} class="sr-only peer">
                        <div class="w-10 h-6 bg-gray-200 peer-focus:ring-2 peer-focus:ring-indigo-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
                    </label>
