<x-app-layout>
    <x-slot name="header">
        <div class="flex items-start gap-3 sm:items-center">
            ...
            <div class="min-w-0">
                <h1 class="text-2xl font-bold text-slate-900">Nueva noticia</h1>
                <p class="mt-1 truncate text-sm text-slate-500">Crea una nueva noticia para el sitio web</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            <form method="POST" action="{{ route('superadmin.noticias.store') }}" enctype="multipart/form-data">
                @csrf

                @include('superadmin.noticias._form')
