            ...
            <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <select name="role"
                        ...
                        <option value="trabajador" {{ old('role', $user->roles->first()?->name) === 'trabajador' ? 'selected' : '' }}>Trabajador</option>
                        <option value="administrador" {{ old('role', $user->roles->first()?->name) === 'administrador' ? 'selected' : '' }}>Administrador</option>
                    </select>
                    @error('role')<p class="mt-1 text-xs text-rose-600">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">Nueva contraseña <span class="font-normal text-slate-400">(dejar en blanco para no cambiar)</span></label>
