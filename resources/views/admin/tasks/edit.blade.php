    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-0.5 truncate max-w-xs">{{ $task->title }}</p>
            </div>
        </div>
    </x-slot>
            ...
            <form method="POST" action="{{ route('admin.tasks.update', $task) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                              class="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $task->description) }}</textarea>
                </div>
